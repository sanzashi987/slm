...
def run_train(args, train_loader, model, mano, mesh_sampler):
    ...
    logger.info(f"Start training: {args.num_train_epochs} epochs, {iters_per_ep} iters/epoch")
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            pred_kp2d_from_mesh = orthographic_projection(
                pred_kp3d_from_mesh, pred_cam)
            pred_kp2d = orthographic_projection(pred_kp3d, pred_cam)

            # ── losses ──────────────────────────────────────────
            l_kp3d = keypoint_3d_loss(crit_kp, pred_kp3d, gt_kp3d, has_kp3d)
            l_kp3d_mesh = keypoint_3d_loss(crit_kp, pred_kp3d_from_mesh, gt_kp3d, has_kp3d)
            l_vert = (args.vloss_w_sub * vertices_loss(crit_vert, pred_v_sub, gt_v_sub, has_mesh) +
                      args.vloss_w_full * vertices_loss(crit_vert, pred_v,     gt_v,     has_mesh))
            l_kp2d = (keypoint_2d_loss(crit_kp, pred_kp2d, gt_kp2d, has_mesh) +
                      keypoint_2d_loss(crit_kp, pred_kp2d_from_mesh, gt_kp2d, has_mesh))

            loss = (args.joints_loss_weight * (l_kp3d + l_kp3d_mesh) +
        ...
        if (epoch + 1) % args.save_every_n_epochs == 0 or epoch == args.num_train_epochs - 1:
...
def run_eval(args, val_loader, model, mano, mesh_sampler):
    ...
    with torch.no_grad():
        for i, batch in enumerate(val_loader):
            images = batch["image"].to(args.device)
            fnames = batch.get("fname", [str(i * args.per_gpu_eval_batch_size + j)
            ...
            for j in range(images.shape[0]):
                ...
                fname  = fnames[j] if isinstance(fnames[j], str) else str(fnames[j].item())
                ...
                if args.export_obj and (i * args.per_gpu_eval_batch_size + j) % args.export_obj_every == 0:
                    stem    = op.splitext(op.basename(fname))[0] if "/" in fname or "." in fname else fname
...
def parse_args():
    ...
    p.add_argument("--export_obj_every", default=1, type=int,
...
def main():
    ...
    if not args.run_eval_only:
        train_dataset = FreiHandDataset(
            root=args.data_root, split="training",
    ...
    val_dataset = FreiHandDataset(
        root=args.data_root, split="evaluation",
